...
- fork 的 release tag 是本仓库自己的 annotated tag，通常指向“合并官方 + fork 改动”的 merge commit；官方同名 tag 指向官方 commit。不要假设两边同名 tag 的 commit 完全一致。
...
git ls-remote --tags upstream 'v*' | awk '{print $2}' | sed 's#refs/tags/##; s#\^{}##' | sort -uV | tail -20
